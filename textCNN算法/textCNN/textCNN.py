...
class TextCNN(object):
    ...
    def __init__(self, n_class=10, embed_dim=64,seq_len=600,n_filter=256,k_size=5,vocab_size=5000,
                 hidden_dim=128,dropout=0.5,lr=1e-3,batch_size=64,epoch=10,verbose=100,save=10):

        self.n_class = n_class # 输出类别数
        self.embed_dim = embed_dim # 词向量维度
        self.seq_len = seq_len # 输入序列长度
        self.n_filter = n_filter # 卷积核数量
        self.k_size = k_size # 卷积核大小
        self.vocab_size =vocab_size # 字符列表大小
        self. hidden_dim = hidden_dim # 全连接层的维度
        self.dropout = dropout # dropout比例
        self.lr = lr # 学习率
        self.batch_size = batch_size # 批次大小
        self.epoch = epoch # 训练轮数
        self.verbose = verbose # 每N轮输出日志
        self.save = save # 每N轮保存模型
    ...
    def __build_model(self):
        ...
        with tf.name_scope('cnn'):
            """CNN层"""
            # conv = tf.layers.conv1d(embedding_inputs, self.n_filter, self.k_size, name='conv')
            # https://tensorflow.google.cn/api_docs/python/tf/layers/conv1d?hl=zh-cn
            conv = tf.keras.layers.Conv1D(self.n_filter, self.k_size, name='conv')(embedding_inputs)
            pool = tf.reduce_max(conv, reduction_indices=[1], name='pool') # 最大池化，按行
            # https://www.jianshu.com/p/0accff741e98
        with tf.name_scope('fc'):
            """全连接层"""
            # fc = tf.layers.dense(pool, self.hidden_dim, name='fc1')
            fc = tf.keras.layers.Dense(self.hidden_dim, name='fc1')(pool)
            fc = tf.contrib.layers.dropout(fc, self.keep_prob) # http://www.imooc.com/article/289687
            fc = tf.nn.relu(fc)
            #self.logits = tf.layers.dense(fc, self.n_class, name='fc2')
    ...
    def train(self,train_file, valid_file):
        ...
        for epoch in range(self.epoch): # 训练轮数
            ...
            for x_batch, y_batch in batch_train:
                feed_dict = {self.input_x:x_batch, self.input_y:y_batch,self.keep_prob:self.dropout}
                # 保存训练过程数据
    ...
    def test(self,test_file):
        """评估模型"""
        # 加载测试集数据
        texts_test, labels_test = read_file(test_file) 
        # 加载字符-id映射表，类别-id映射表
        ...
        saver = tf.train.Saver()
        saver.restore(sess=session,save_path=BEST) # 读取保存的最佳模型
        """
        loss_test, acc_test = self.__evaluate(self.session, x_test, y_test) # 在测试集上评估
        #loss_test, acc_test = self.__evaluate(session, x_test, y_test) # 在测试集上评估
